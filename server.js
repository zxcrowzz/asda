...
// Assuming you have already set up your WebSocket server
...
wss.on('connection', (ws) => {
    ...
    // Assign a unique ID for each connection
    ws.id = uuidV4();
    ...
    wss.clients.forEach(client => {
        if (client !== ws && client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify({
                type: 'user-connected',
                id: ws.id // Send the ID of the new user
            }));
        }
    });
    ...
    ws.on('message', (message) => {
        ...
        switch (data.type) {
            case 'video-offer':
                // Send the video offer to the specific client who needs it
                wss.clients.forEach(client => {
                    if (client.id === data.id && client.readyState === WebSocket.OPEN) {
                        client.send(JSON.stringify({
                            type: 'video-offer',
                            sdp: data.sdp,
                            id: ws.id // Send the ID of the sender
                        }));
                    }
                });
                break;
            ...
            case 'video-answer':
                // Send the video answer back to the client who initiated the offer
                wss.clients.forEach(client => {
                    if (client.id === data.id && client.readyState === WebSocket.OPEN) {
                        client.send(JSON.stringify({
                            type: 'video-answer',
            ...
            case 'ice-candidate':
                // Send the ICE candidate to the specific client who needs it
                wss.clients.forEach(client => {
                    if (client.id === data.id && client.readyState === WebSocket.OPEN) {
                        client.send(JSON.stringify({
                            type: 'ice-candidate',
